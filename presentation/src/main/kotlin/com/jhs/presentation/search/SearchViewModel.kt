...
class SearchViewModel @Inject constructor(
    ...
    fun fetchBooks(query: String, sort: String) {
        ...
        fetchJob = viewModelScope.launch {
            getDocumentsUseCase(query, sort)
                .collect { result ->
                    when (result) {
                        ...
                        is DataResource.Success -> {
                            ...
                            val uiItems = searchResult.documents.map { doc ->
                                doc.toPresentation().copy(
                                    isBookmarked = searchResult.bookmarkedIsbns.contains(doc.isbn)
                                )
                            }
                        ...
                        is DataResource.Error -> {
                            dispatch(
                                SearchContract.Event.OnSearchFailed(
                                    result.throwable.message ?: "데이터를 불러오지 못했습니다."
                                )
                            )
                        }
                    }
